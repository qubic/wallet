 ...
 */
  ...
  public removeTransaction(tx: Transaction): void {
    const list = this.internalTransactions.getValue();
    const updatedList = list.filter(t => t.id.slice(0, 56) !== tx.id.slice(0, 56));
    this.internalTransactions.next(updatedList);
  }
}
